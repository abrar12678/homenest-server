...
async function getAdminStats(req: any, res: any): Promise<void> {
  try {
    ...
    const serializedProps = recentProperties.map((p: any) => ({
      ...p,
      _id: p._id.toString(),
      postedBy: p.postedBy?.toString?.() || String(p.postedBy),
    }));
    ...
    const monthlyProperties = await db
      .collection("properties")
      .aggregate([
        { $match: { createdAt: { $gte: sixMonthsAgo.toISOString() } } },
        {
    ...
    const inquiryPropIds = [
      ...new Set(
        recentInquiries
          .map((i: any) => i.propertyId?.toString())
          .filter(Boolean),
      ),
    ];
    const inquiryUserIds = [
      ...new Set(
        recentInquiries
          .map((i: any) => i.fromUserId?.toString())
          .filter(Boolean),
      ),
    ];
    const [inqProps, inqUsers] = await Promise.all([
      inquiryPropIds.length > 0
        ? db
            .collection("properties")
            .find({
              _id: { $in: inquiryPropIds.map((id: any) => new ObjectId(id)) },
            })
            .project({ title: 1 })
            .toArray()
        : [],
      inquiryUserIds.length > 0
        ? db
            .collection("users")
            .find({
              _id: { $in: inquiryUserIds.map((id: any) => new ObjectId(id)) },
            })
            .project({ name: 1 })
            .toArray()
        : [],
    ]);
    const inqPropMap = new Map(
      inqProps.map((p: any) => [p._id.toString(), p.title]),
    );
    const inqUserMap = new Map(
      inqUsers.map((u: any) => [u._id.toString(), u.name]),
    );
    ...
    sendSuccess(res, {
      ...
      monthlyUsers: monthlyUsers.map((m: any) => ({
        month: m._id,
        count: m.count,
      })),
      monthlyProperties: monthlyProperties.map((m: any) => ({
        month: m._id,
        count: m.count,
      })),
      recentInquiries: recentInquiries.map((i: any) => ({
        ...
        propertyTitle:
          inqPropMap.get(i.propertyId?.toString?.() || "") || "Unknown",
        fromUserName:
          inqUserMap.get(i.fromUserId?.toString?.() || "") || "Unknown",
      })),
    });
  } catch (error: any) {
  ...
  try {
    ...
    if (!role || !validRoles.includes(role))
      return sendError(
        res,
        400,
        "Invalid role. Must be user, agent, or admin.",
      );
    ...
    sendSuccess(res, {
      user: sanitizeUser({ ...result, _id: result._id.toString() }),
    });
  } catch (error: any) {
...
async function deleteUser(req: any, res: any): Promise<void> {
  try {
    ...
    if (propIds.length > 0) {
      await db
        .collection("properties")
        .deleteMany({ postedBy: new ObjectId(id) });
      await db
        .collection("reviews")
        .deleteMany({ propertyId: { $in: propIds } });
    }
    await db.collection("reviews").deleteMany({ userId: new ObjectId(id) });
    await db.collection("inquiries").deleteMany({
      $or: [{ fromUserId: new ObjectId(id) }, { toUserId: new ObjectId(id) }],
    });
...
async function deleteProperty(req: any, res: any): Promise<void> {
  try {
    ...
    await db
      .collection("inquiries")
      .deleteMany({ propertyId: new ObjectId(id) });
    await db
...
async function getReviews(req: any, res: any): Promise<void> {
  try {
    ...
    // Get property titles
    const propIds = [
      ...new Set(reviews.map((r: any) => r.propertyId.toString())),
    ];
    const props =
      propIds.length > 0
        ? await db
...
async function getPayments(req: any, res: any): Promise<void> {
  try {
    ...
    const userIds = [
      ...new Set(
        payments.map((p: any) => p.userId?.toString()).filter(Boolean),
      ),
    ];
    const propIds = [
      ...new Set(
        payments.map((p: any) => p.propertyId?.toString()).filter(Boolean),
      ),
    ];
...
async function getInquiries(req: any, res: any): Promise<void> {
  try {
    ...
    const inquiryPropIds = [
      ...new Set(
        inquiries.map((i: any) => i.propertyId?.toString()).filter(Boolean),
      ),
    ];
    const fromUserIds = [
      ...new Set(
        inquiries.map((i: any) => i.fromUserId?.toString()).filter(Boolean),
      ),
    ];
    const toUserIds = [
      ...new Set(
        inquiries.map((i: any) => i.toUserId?.toString()).filter(Boolean),
      ),
    ];
    ...
    const [props, fromUsers, toUsers] = await Promise.all([
      ...
      fromUserIds.length > 0
        ? db
            .collection("users")
            .find({
              _id: { $in: fromUserIds.map((id: any) => new ObjectId(id)) },
            })
            .project({ name: 1, email: 1 })
            .toArray()
        : [],
      toUserIds.length > 0
        ? db
            .collection("users")
            .find({
              _id: { $in: toUserIds.map((id: any) => new ObjectId(id)) },
            })
            .project({ name: 1, email: 1 })
            .toArray()
        : [],
    ...
    const propMap = new Map(props.map((p: any) => [p._id.toString(), p.title]));
    const fromUserMap = new Map<string, { name: string; email: string }>(
      fromUsers.map((u: any) => [
        u._id.toString(),
        { name: u.name, email: u.email },
      ]),
    );
    const toUserMap = new Map<string, { name: string; email: string }>(
      toUsers.map((u: any) => [
        u._id.toString(),
        { name: u.name, email: u.email },
      ]),
    ...
    sendSuccess(res, {
      inquiries: inquiries.map((i: any) => ({
        ...
        toUserId: i.toUserId?.toString?.() || String(i.toUserId),
        propertyTitle:
          propMap.get(i.propertyId?.toString?.() || "") || "Unknown",
        fromUserName:
          fromUserMap.get(i.fromUserId?.toString?.() || "")?.name || "Unknown",
        fromUserEmail:
          fromUserMap.get(i.fromUserId?.toString?.() || "")?.email || "",
        toAgentName:
          toUserMap.get(i.toUserId?.toString?.() || "")?.name || "Unknown",
      })),
      total,
      page,
      totalPages: Math.ceil(total / limit),
